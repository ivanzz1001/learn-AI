...
1. `~/.openclaw/agents/`
    ...
    说明：存放每个智能体独立的会话状态、会话历史和认证配置。

1. `~/.openclaw/credentials/`

    用途：API凭证存储（旧版）

    说明：存放API Key和OAuth凭证的旧目录。新版凭证更推荐使用`agents/<agentId>/agent/auth-profiles.json`

1. `~/.openclaw/secrets.json`

    用途：加密凭证存储。

    说明：用于存储加密后的敏感信息，是`credentials/`的加密替代方案，安全性更高

1. `~/.openclaw/memory/`

    用途：向量记忆库

    说明：存放用于长期记忆搜索的向量索引，以SQLite数据库形式存储

## 6. 错误排查(Troubleshooting)

#### 6.1 错误1：npm.ps1 cannot be loaded

原因：PowerShell 默认禁止脚本执行

解决：调整 PowerShell 执行策略 的，目的是允许运行本地脚本
```bash
...
>ps: 通常我们以“管理员身份”运行控制台即可成功安装

#### 6.2 错误 2：node-llama-cpp 反复 Clone llama.cpp / 编译失败
...
#### 6.3 错误 3：ENOENT: Could not read package.json
...
#### 6.5 Gateway 无法访问（127.0.0.1 refused）
...
然后访问：http://127.0.0.1:18789 或 http://localhost:18789
